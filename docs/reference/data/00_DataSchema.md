...
**Existing code dependency this change affects**: `lib/models/trash_entry.dart`'s doc comment states `TrashEntry.createdAt` (used for the Trash info popup's "제작된 날짜/시간" display) currently maps directly from `StyleLog.wornDate`, since StyleLog had no `createdAt` of its own before now. Now that it does, and `wornDate` is nullable, that mapping should switch to the new `StyleLog.createdAt` — otherwise a StyleLog with a null `wornDate` would show a blank "제작된 날짜" in Trash. This is an implementation-stage change to `trash_providers.dart`, not a schema concern, but flagged here since it's a direct, concrete consequence of this exact edit — see Open Question #18.
...
- Instead, at first launch, the app uses a **fixed local placeholder scope, `__unlinked_local__`**, in place of a real `uid` — i.e. all reads/writes go to `users/__unlinked_local__/clothingItems/...` etc., using the exact same collection paths and field shapes as every other section of this document (nothing about the per-collection schemas changes; only the path segment is a placeholder instead of a real Auth `uid`).
...
2. The app reads every document currently cached locally under `users/__unlinked_local__/...` (still a pure local read, no network needed for this step) and re-writes each one under `users/{realUid}/...` — same collections, same field shapes, just a different path prefix. This is a bulk copy, not a transformation.
3. Once the copy completes, the local `users/__unlinked_local__/...` documents are deleted (cleanup — nothing should be left behind pointing at the placeholder scope).
4. `Firestore.instance.enableNetwork()` is called, and the newly `{realUid}`-scoped writes sync to the cloud normally from this point on, per the rest of this document's design (§1 topology, §7 wearCount batching, etc. — nothing about those designs changes; they simply start actually reaching a server instead of only existing locally).
...
18. **`TrashEntry.createdAt` mapping needs to move off `StyleLog.wornDate` (§5)** — `lib/models/trash_entry.dart`'s doc comment confirms `TrashEntry.createdAt` (Trash popup's "제작된 날짜/시간") currently sources from `StyleLog.wornDate` (StyleLog had no `createdAt` before 2026-08-04). With `wornDate` now nullable, this mapping must switch to the new `StyleLog.createdAt` instead, or Trash would show a blank date for any style log without a recorded worn date. Implementation-stage fix to `trash_providers.dart` (or wherever this mapping lives) — not a schema-doc concern itself, but a concrete, direct consequence of this review's change that shouldn't get lost.
