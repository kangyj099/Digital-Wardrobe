...
① Clothing Archiving		
(Auto categorization + location memo)	Solves the problem of not knowing what clothes are owned and being unable to physically find them.	
Tag-based browsing + memos (e.g., “box in 3rd closet compartment”) ensures no item is ever “lost.”	Essential — without this, there is no reason for the app to exist	
③ Outfit Archiving		
(OOTD logging + history)	Breaks habitual outfit patterns and enables users to reflect on past outfits, understand preferences via wear frequency, and revisit moods/styles	Retention core — not strictly required, but without it user interest drops significantly, reducing long-term engagement
