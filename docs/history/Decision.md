...
결정:
- **`documentation-conventions` 스킬에 "훑을 수 있게 쓰기" 절 신설** — `docs/reference/**` 산문에 적용되는 6원칙: (1) 결론을 문단 첫 줄에, (2) 한 문장에 한 명제만, (3) 대시(`—`)로 절을 잇지 않음(표·목록 구분자로만), (4) 볼드는 랜드마크로만(문단·항목 이름 + 문장 안 핵심 구절, 개수가 적어야 기능), (5) 밀도를 균일하게 만들지 않음(짧은 줄과 긴 줄 혼합), (6) 헤딩은 명사구로("어떻게 적나"가 아니라 "기재 요령" — 조사·어미가 없어 grep과 §참조가 안정적).
- **항목화 규칙** — 규칙이나 항목이 셋 이상이면 불렛으로 나열한다. 한 규칙의 근거·부연은 하위 줄로 들여쓴다. 형제 불렛으로 올리면 독립 규칙으로 읽혀 적용 조건이 사라진다.
- **표 안의 서술 규칙** — 서술은 셀 안에 두고 각주나 별도 절로 빼지 않는다. 셀 안에서 한 문장마다 `<br>`로 줄을 나눠 원칙 (2)를 지킨다. `00_OwnershipMap.md`가 기준 사례. 초안에서는 셀에 주 번호만 남기고 "비고 상세" 절로 분리했으나, 사용자가 표를 읽다가 다른 절로 튀는 비용이 더 크다고 판단해 되돌렸다.
- **§1.5 Concise Writing과 충돌하지 않는다** — 위 규칙은 문장 모양과 배치만 바꾸며 중복을 늘리지 않는다. 사용자가 "같은 내용 반복은 나도 싫어한다, 정본 문서는 설명문이 아니다"라고 명시해 redundancy는 의도적으로 제외했다.
- **`00_OwnershipMap.md` 재작성** — 정보 손실 0, 중복 추가 0. 구조 변경만: 표를 규칙 블록보다 위로 이동(문서를 여는 목적이 "누가 소유하는가" 조회이므로), 비고 열 산문 10건을 대시 연결에서 `<br>` 분리 단문으로 재작성, 규칙 블록을 "무엇을 등재하나 / 무엇을 제외하나 / 어떻게 적나 / 어떻게 검증하나" 4개 소절로 분할.

사유:
사용자가 `00_OwnershipMap.md`를 두고 "너무 클로드 말투다, 사람도 알아보기 쉽게 재작성 가능한가, 사람이 보기 편한 문서와 클로드가 보기 편한 문서가 배타적인가"라고 물었고, 이어 "내용이 왜 이렇게 눈에 안 들어오나, GPT가 쓴 건 너저분해도 잘 보이는 편인데"라고 원인을 물었다.

진단: **밀도가 균일해서 눈이 착지할 지점이 없다.** 모든 문장이 비슷하게 조밀하고 비슷하게 중요하면 건너뛸 문장이 없고, 훑기가 불가능해져 정독만 남는다. "너저분한데 잘 보인다"는 역설이 이를 가리킨다 — 불균일함 자체가 시각적 랜드마크다. 부수 원인 넷: 한 문장에 주장·근거·대조를 함께 실음, 대시로 절을 이어 한 문장이 서너 줄이 됨(한국어는 술어가 끝에 와서 판정이 계속 뒤로 밀리므로 영어보다 타격이 큼), 볼드를 문장 중간 의미 강조에 써서 랜드마크가 안 됨, 결론이 문단 끝에 옴.

배타성 판단: **배타적이지 않다.** 두 독자가 요구하는 것(정확한 용어, 단일 정본, 중복 없음, 스캔 가능한 표)이 대부분 겹친다. 갈리는 축은 근거절의 위치 하나인데, 에이전트는 파일 전체를 한 번에 컨텍스트에 넣어 배치에 무감각하고 사람은 순차로 읽어 배치에 민감하므로, 정보를 빼지 않고 위치만 바꿔 둘 다 만족시킬 수 있다. 근거 문장 자체는 삭제 불가 — Audit 3라운드를 거치며 굳은 우회 방지 문장이라 지우면 그 검증이 무효가 된다. 리포지토리 내 사람 친화 스타일 선례는 사용자가 직접 쓴 `docs/reference/design/features/style_log/filter.md`(짧은 단정문, 사용/금지 분리).

Impact:
- `.claude/skills/documentation-conventions/SKILL.md` — "훑을 수 있게 쓰기" 절 추가. §1.4/§1.5 verbatim 블록은 손대지 않음.
- `docs/reference/architecture/00_OwnershipMap.md` — 전면 재작성(내용 동일, 구조·문장 형태만 변경). 표 19행과 미등재 6항목 모두 보존.
- 향후 `docs/reference/**` 신규 작성·수정 전부에 적용. History 문서(`Decision.md`/`TechnicalDebt.md`)는 §1.3에 따라 상세 우선이므로 대상 아님.
- 코드 변경 없음.
