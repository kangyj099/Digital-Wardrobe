...
- **BACKLOG.md 진행기록 커밋을 단독 커밋 대신 작업 커밋 하나로 합칠 수 있는 개선안 논의** (2026-07-13). 지금은 CLAUDE.md "BACKLOG.md 커밋 승인 차등" 규칙을 "단독 커밋 원칙"으로 단순화해뒀지만(Worker가 Review 판정보다 먼저 커밋을 확정해버려서 묶을 대상이 없는 구조적 문제 때문), 근본적으로 Worker→Review 파이프라인 자체를 바꿔서(예: Worker의 커밋 시점을 Review 통과 이후로 늦추는 등) 완료 서술까지 포함한 단일 커밋이 실제로 가능하게 만들 여지가 있는지는 아직 논의 안 됨 — 별도 세션에서 검토.
