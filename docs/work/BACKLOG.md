...
**8페이지 레이아웃&데이터 완전성 감사 전체 완료(2026-08-12)** — 옷장 메인→옷 상세→코디 메인→코디 상세→스타일일지 메인→스타일일지 열람→설정→삭제&휴지통, 스펙↔구현 전체 대조. 설정: 발견 0건. 삭제&휴지통: 발견 다수(아래 Current로 이관, 즉시 구현 착수 확정). 그 외 페이지 결정사항은 `Decision.md` 참고.

---

# Current

**삭제&휴지통 감사 발견사항 — 전부 즉시 구현 착수 확정(2026-08-12, 사용자 지시)**:

*Track A(UI/Screen 픽스, 스냅샷 의존 없음, M)*: (1) 휴지통 정보 팝업에 가시적 닫기(X) 버튼 추가 (2) "N일" 표시를 이미지 안쪽 중앙 하단 오버레이로 이동(현재는 상단 제목 텍스트 병기) (3) 정보 팝업의 [복원]/[영구삭제] 버튼을 스크롤과 무관하게 하단 고정 (4) `CompositionGalleryTile`에 "연결끊김" 배지 추가(삭제된 옷 포함 코디 식별) (5) 상세화면 3곳(옷 상세/코디 상세/스타일일지 열람, "⋯"→삭제)의 `GlassToast`에 `actionLabel: '실행취소'`/`onAction` 배선 — 메인 갤러리 다중선택 삭제 3곳은 이미 구현돼 있어 그 패턴 재사용.

*Track B(코디 스냅샷 아키텍처, Data/Architecture × Decision 먼저 — §12.1 규정상 크기 무관 Audit 게이트 필수)*: 스펙(`05_삭제 & 휴지통...md` "옷 삭제 시 코디 캐스케이드 처리")의 핵심 전제 "코디는 저장 시마다 평면 렌더링 스냅샷 이미지 저장"이 실제로 미구현 — `Composition.coverImagePath` 필드는 있으나 아무도 안 채움(코디 갤러리 타일이 텍스트 전용인 기존 P1과 동일 원인). 이걸 구현해야 파생되는 항목들(코디 상세 스냅샷 표시+안내문구, 편집 진입 시 삭제된 옷 확인팝업+자동제거+스냅샷 재생성, 0개 남으면 미완성 처리)도 같이 풀림. Decision(캡처 방식/저장 위치/기존 asset 경로와의 공존 방식) 확정 후 L 구현.
