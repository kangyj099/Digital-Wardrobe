...
1. **"Step⑦ 나머지 스코프" 진행 중 — 4개 그룹(A~D)으로 분해, 그룹 A 완료(2026-07-19), 그룹 B부터 이어가면 됨.**
   ...
   - **그룹 C(설정 나머지 — 알림/다크모드/휴지통 진입/로그아웃)**: 스펙 확정(2026-07-27, `docs/history/Decision.md` "설정 화면 최종 로우 구성 확정" 항목/`04_설정.md` §2·§4·§6) + 그 결정문서 커밋 완료(`d95d32e`). **Task 1(화면 로우 재구성) 진행 중 — Worker 구현 완료 + Review 1회 PASS(P3 1건, PM이 직접 수정 완료), Tester 아직 미실행, 커밋도 아직 안 함.** 토큰 예산 문제로 세션 종료, 다음 세션이 바로 이어받을 것.
     - **Task 1 현재 워킹트리 상태(미커밋)**: `lib/screens/settings_screen.dart`(프로필편집 삭제, 휴지통 로우 추가 — 일반 섹션 마지막·체브론·`context.push(AppRoute.trashMain)`, 로그아웃 로우 추가 — 계정 섹션·`colorScheme.error`·아이콘 병기·체브론 없음) 수정, `lib/widgets/undoable_action_toast.dart` 신규(C7 `UndoableActionToast`의 코드베이스 내 첫 구현 — 상태 스토어 없이 SnackBar+undo 콜백+타이머만, 향후 그룹 B 레코드 단위 삭제-Undo가 재사용 가능), `integration_test/closet_main_screen_test.dart`·`integration_test/settings_trash_shell_test.dart` 수정(프로필편집 assertion→휴지통 진입 이동 확인/로그아웃 SnackBar 노출·undo 확인으로 교체). Review가 git diff로 직접 검증 완료(PASS, findings 없음 — P3 analyzer 스타일 힌트 1건은 PM이 즉시 수정해 해소).
     - **다음 세션 즉시 할 일**: (1) Tester를 이 Task 1에 대해 돌린다(Review 통과했으니 정상 절차). (2) Tester까지 통과하면 `feature/flutter-hifi-screens`에 커밋(Task 크기 S/M이라 Audit 없이 완료 처리 가능). (3) 커밋 후에만 **Task 2(다크모드 실동작 배선, Layer=Logic/Feature)** 착수 — `SettingsScreen`의 다크모드 `Switch`(현재 로컬 `bool` state만 토글, `main.dart`가 `ThemeMode.light` 고정)를 Riverpod provider로 끌어올려 `MaterialApp.themeMode`에 연결. `AppTheme.dark` 토큰은 이미 존재. **주의: 이 Task는 Task 1과 같은 파일(`settings_screen.dart`의 다크모드 Switch)을 건드리므로 순서 무관이 아니라 Task 1 커밋 이후 순차 착수해야 함**(이전 기록의 "순서 무관"은 착수 전 재검토 결과 정정됨 — 두 Task가 같은 파일을 건드리는 걸 확인).
