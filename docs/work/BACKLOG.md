...
1. **"Step⑦ 나머지 스코프" 진행 중 — 4개 그룹(A~D)으로 분해, 그룹 A 완료(2026-07-19), 그룹 B부터 이어가면 됨.**
   ...
   - **그룹 C(설정 나머지 — 알림/다크모드/휴지통 진입/로그아웃)**: **스펙 확정 완료(2026-07-27)**, Worker 구현 착수 가능. 사용자 결정: "프로필 편집" 제외(이 앱에 프로필/로그인 개념 없음), 다크모드 로우 유지, 휴지통 진입 로우 신설, 로그아웃 로우 신설. 근거는 `docs/history/Decision.md` "설정 화면 최종 로우 구성 확정" 항목, 갱신된 스펙은 `04_설정.md` §2(로우 4개)/§4(휴지통 스코프 인)/§6.
     - **구현 태스크 체크리스트** (Layer=UI/Screen, Stage=Implementation):
       1. `lib/screens/settings_screen.dart`: "계정" 섹션의 "프로필 편집" 로우 삭제. "일반" 섹션에 휴지통 진입 로우 추가(체브론, `onTap: () => context.push(AppRoute.trashMain)`). "계정" 섹션에 로그아웃 로우 추가(Error 롤 색상+아이콘, Toast+Undo 패턴 — `04_설정.md` §3 기존 승인 로직 그대로).
       2. `integration_test/closet_main_screen_test.dart`(862-878행)와 `integration_test/settings_trash_shell_test.dart`(171-182행): "프로필 편집" 렌더링/탭 assertion 제거, 휴지통 진입/로그아웃 로우 assertion으로 교체.
       3. (별도 태스크, Layer=Logic/Feature) **다크모드 토글 실동작 배선**: 현재 `SettingsScreen`의 다크모드 `Switch`는 로컬 `bool` state만 토글하고 실제 테마는 안 바뀜(`main.dart`가 `themeMode: ThemeMode.light` 고정). `AppTheme.dark` 토큰은 이미 존재 — Riverpod provider로 상태를 끌어올려 `MaterialApp.themeMode`에 연결하는 작업. 그룹 C 화면 구현과 별개로 진행 가능(순서 무관).
