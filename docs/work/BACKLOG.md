...
**하네스 로깅 보강 — 완료 (2026-07-12).** 토큰 소모 진단 중 발견한 두 공백을 메움: (1) `Workflow_Project.md` §14.1 — 에이전트 스폰 전/handoff 직후 `/context` 근사 토큰 Delta를 `docs/work/TokenLog.md`에 기록. (2) §14.2 — Worker/Review/Tester handoff에 Read/Search/Edit 호출 통계(Stats) 의무화, `docs/work/AgentStats.md`에 누적. `worker.md`/`review.md`/`tester.md` 3종 handoff 포맷에 Stats 줄 추가. 부수: stale worktree 4개 정리(위 Known Issues 참고).
