...
**Background button/swatch decorative restyle (2026-07-27).** User asked to replace the flat outline border on `_backgroundColorButton()`/`_backgroundSwatch()` with a soft drop shadow + highlight glow + inner glow. First attempt stacked all three as `BoxDecoration.boxShadow` entries alongside `color` — didn't work, because `BoxDecoration.paint()` draws `boxShadow` before `color`, so the opaque fill painted over everything (Flutter SDK source, `box_decoration.dart`). Redone following this project's existing `GlassPill`/`GlassCircleButton` pattern: a shadow-only outer `Container` (no `color`) wraps a `ClipOval` → `Stack` whose bottom layer is a `DecoratedBox(color: ...)` (not `ColoredBox`/`Container(color:...)`, which is what `ColoredBox` compiles to internally — either would collide with an existing test's `find.byType(ColoredBox)` single-match on the canvas background) and whose top layer is a `RadialGradient` inner-glow overlay painted on top of the fill. Known follow-up: the inner glow reads as a visible white band on light swatches (white/light-gray) instead of blending — logged in `docs/history/TechnicalDebt.md` (P3, functionality unaffected, deferred by user). 29/29 tests still pass.

The rest of this document (all Round 1/Round 2 task specs below) reflects completed, shipped work and does not need to be re-read to resume — the Composition Artboard widget feature is functionally complete; the one open item is the cosmetic inner-glow-on-light-colors follow-up tracked in TechnicalDebt.md.
