...
- `docs/` — 나(Claude)에게 보여주려고 만든 폴더. 기획/디자인/워크플로우 레퍼런스와 스킬 정의가 있음. 적극적으로 참고할 것.
  - `docs/knowledge/reference/plan/` — 기획 문서 (MVP, Needs, IA/UserFlow, 화면별 UX명세서)
  - `docs/knowledge/reference/design/` — 디자인 원칙, 브랜드가이드
  - `docs/knowledge/reference/policy/` — 워크플로우 정책 3종 (Project/Design/Development)
  - `docs/knowledge/reference/Glossary.md` — 전 영역 공통 용어집 (특정 카테고리에 속하지 않음)
  - `docs/knowledge/Skills/UIUX/` — UIUX 스킬 정의
  - `docs/knowledge/history/Decision.md` — 의사결정 기록
  - `docs/knowledge/history/TechnicalDebt.md` — 기술부채 기록
...
이 루트 세션이 PM 역할을 맡는다. 구현은 `.claude/agents/worker.md`, 리뷰는 `.claude/agents/review.md`, 런타임 동작 검증은 `.claude/agents/tester.md` 서브에이전트에게 위임한다. Tester는 Review 통과 후에만 투입되고, Write 권한은 `integration_test/` 하위로만 제한된다(`lib/`는 절대 건드리지 않음). 상세 근거는 `docs/knowledge/reference/policy/Workflow_Project.md`(특히 §3, §12)를 따른다.

**필수 체크포인트 (건너뛰지 말 것):**

1. **작업 분배 가시성**: 워커/리뷰/테스터 서브에이전트를 스폰하기 전에, 누구(에이전트 타입)에게 어떤 task를 왜 보내는지 평문으로 먼저 알린다. 승인을 기다릴 필요는 없다(비차단 로그).
2. **결정문서 diff 확인**: `docs/knowledge/history/*`, `docs/knowledge/reference/**` 수정은 항상 일반 Edit 승인 흐름을 거친다. 이 경로에 대해 auto-accept를 절대 켜지 않는다.
...
4. **작업↔리뷰↔테스트 사이클**: `Worker → Review → (통과 시) Tester → (미통과 시) Worker(수정)` 순환은 사람 개입 없이 알아서 돌리되, 사이클이 끝나면 "로그 보시겠어요?"를 사용자에게 묻는다 (자동으로 전체 로그를 쏟아내지 않음). Review가 미통과면 Tester로 가지 않고 바로 Worker(수정)로 돌아간다. Tester가 통과하면 그대로 완료 — 수정 후 재검증 루프는 자동으로 돌지 않는다. Tester는 런타임 동작이 있는 작업이면 Task 크기와 무관하게 매번 돌린다(Review 통과 후에만).
