...
7. **완료 후 연속성 시뮬레이션 & `/clear` 권유**: Task 단계가 완료됐고 이 세션에서 더 이어갈 작업이 없다고 판단되면, `docs/work/BACKLOG.md`(및 방금 추가된 Decision.md/TechnicalDebt.md 항목)만으로 새 세션이 정확히 이어받을 수 있는지 시뮬레이션한다. 문제 없으면 사용자에게 평문으로 `/clear`를 권유한다(직접 실행은 하지 않음) — Task 크기 무관 적용. 상세 근거는 `Workflow_Project.md` §3 "Post-Completion Continuity Simulation & `/clear` Recommendation" 참고.
