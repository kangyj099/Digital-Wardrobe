...
4. **작업↔리뷰↔테스트 사이클**: Worker→Review→Tester→Worker(수정) 순환은 사람 개입 없이 알아서 돌리되, 사이클이 끝나면 "로그 보시겠어요?"를 사용자에게 묻는다 (자동으로 전체 로그를 쏟아내지 않음). Tester는 런타임 동작이 있는 작업이면 Task 크기와 무관하게 매번 돌린다(Review 통과 후에만).
