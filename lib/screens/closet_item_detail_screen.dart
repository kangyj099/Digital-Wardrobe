...
class ClosetItemDetailScreen extends ConsumerWidget {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '${item.category?.label ?? '미분류'} · ${item.season?.label ?? '미분류'} · ${item.material?.label ?? '미분류'} · ${item.color?.label ?? '미분류'}',
  ...
  Future<void> _confirmAndDelete(BuildContext context, WidgetRef ref, String itemId, int linkedCount) async {
    ...
    // [Review P0 수정] `context.pop()` 이후 팝된 화면의 element는 dispose되고, `ref`는 그
    // element에 묶여 있어 나중(토스트 액션 탭 시점)에 `ref.read(...)`를 호출하면
    // `ConsumerStatefulElement._assertNotDisposed()`가 release 빌드에서도 `StateError`를
    // 던진다(`ref`가 화면보다 오래 산다는 가정이 틀렸음 — Review가 `flutter_riverpod` 실제
    // 소스로 확인). `ref`가 아니라 notifier 객체 자체를 pop 이전에 미리 캡처해 재사용한다 —
    // `StateNotifier`는 위젯과 독립적으로 살아있다.
    final notifier = ref.read(closetItemsProvider.notifier);
    notifier.softDeleteMany({itemId});
    context.pop();
    ...
    GlassToast.show(
      ...
      message: '휴지통으로 이동됨',
      actionLabel: '실행취소',
      onAction: () => notifier.restoreMany({itemId}),
    );
  }
}
