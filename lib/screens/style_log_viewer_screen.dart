...
/// 2026-07-15-step7-detail-binding.md` Global Constraints 참고). 연결된 코디가 있으면
/// (스타일일지는 코디를 최대 1개만 연결하므로) 텍스트 칩 대신 `CompositionPreviewCard`
/// 하나를 본문 하단에 직접 그린다(캐러셀 불필요).
class StyleLogViewerScreen extends ConsumerWidget {
  ...
  Future<void> _bindComposition(BuildContext context, WidgetRef ref) async {
    ...
    // `composition_detail_screen.dart`의 `_bindStyleLog`에는 없던 mounted 가드를 여기서는
    // 추가한다 — async 갭 이후 ref를 쓰기 전에 위젯이 여전히 살아있는지 확인
    // (`docs/history/TechnicalDebt.md` 참고, `_bindStyleLog` 쪽은 별도로 정리 예정).
          ...
          children: [
            AspectRatio(
              ...
              child: ClipRRect(
                ...
                // `style_log_gallery_tile.dart`와 동일 가드 — coverImagePath가 비어 있으면
                // (미완성 스타일일지, `isIncomplete` 후속 작업 전까지의 임시 데이터 등)
                // Image.asset("")가 asset 로드 예외를 던지므로 회색 placeholder로 대체한다.
                child: log.coverImagePath.isEmpty
