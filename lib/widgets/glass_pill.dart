...
/// primitive. 이전에 `OverlayHeader`/`FrostedBackButton`이 각자 따로 구현하던 블러/보더/
/// 그림자 값을 여기로 추출했다 — 카테고리 토글, 계절 필터, "선택" 버튼 등 floating pill
/// control은 전부 이 위젯 하나로 개별 감싸야 하며, 여러 컨트롤을 하나의 [GlassPill] 안에
/// 함께 담지 않는다(Pinned Rule 위반).
///
...
class GlassPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 클립 경계에서 그림자가 잘려 보이지 않는다(OverlayHeader의 기존 구조 그대로 계승).
    return Container(
